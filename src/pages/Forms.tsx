  ...
  return (
    <>
      <PageHeader
        ...
        actions={<Btn variant="primary"><Plus className="w-3.5 h-3.5" /> New form</Btn>}
      />
      <PageBody>
        <div className="grid grid-cols-3 gap-3 mb-6">
          {forms.map((f) => (
            <div key={f.id} className="border-hairline rounded-lg bg-card p-4 hover:bg-surface-hover transition-colors cursor-pointer">
        ...
        <div className="border-hairline rounded-lg bg-card">
          ...
          <div className="grid grid-cols-[2fr_1.5fr_1fr_1.2fr_auto] px-4 h-9 items-center text-xs text-muted-foreground font-medium border-b-hairline bg-surface/50">
            <div>Contact</div><div>Service</div><div>Postcode</div><div>Date</div><div></div>
          </div>
          {formSubmissions.map((s) => (
            <div key={s.id} className="grid grid-cols-[2fr_1.5fr_1fr_1.2fr_auto] px-4 h-11 items-center text-sm border-b-hairline last:border-b-0 hover:bg-surface-hover transition-colors">
              ...
              <Btn className="h-7">Create job <ArrowRight className="w-3 h-3" /></Btn>
            </div>
          ))}
        </div>
      </PageBody>
